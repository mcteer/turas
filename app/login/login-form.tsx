...
export function LoginForm({ error, returnTo }: { error: boolean; returnTo?: string }) {
  return <form action="/api/auth/login" className="w-full max-w-sm space-y-5 rounded-xl border bg-card p-7 shadow-sm" method="post">
    <div className="space-y-2"><p className="text-sm font-medium text-muted-foreground">Turas Command Center</p><h1 className="text-2xl font-semibold tracking-tight">Synthetic interview demo</h1><p className="text-sm text-muted-foreground">Sign in with the reviewer credentials supplied for this isolated demonstration.</p></div>
