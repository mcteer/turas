...
- Historical failure: hosted Preview `2b55b9d` built successfully at `https://turas-jhb6w626k-turas-6e414af3.vercel.app` (deployment `dpl_4gXyLUc1MVpYejdQuWxz5QkGj3KN`). Authentication failed closed: valid reviewer credentials reached session registration, where the configured database credentials were rejected. Logs recorded `phase=register_session`, `reason=database_authentication`. The previously verified `a844b51` Preview failed the same way.
- Recovery verified on `57a4f94` at `https://turas-cjwvgi4np-turas-6e414af3.vercel.app`: after the owner replaced the expired Preview database connection string, reviewer login, protected customer routes, the 147-record inventory, dated Notion evidence, mobile layout, empty/directory-only states, live Notion response and transcript reload passed. The response distinguishes public evidence from unavailable internal engagement facts and contains no invented handoff details. Test-created chat archival and logged-out access denial also passed.
- No Production environment changes or promotion occurred. Hosted owner approval/replay and the remaining security work are still outstanding.
